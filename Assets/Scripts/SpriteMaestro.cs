...
public class SpriteMaestro : MonoBehaviour
{
    ...
    private void UpdateUI(int newScore)
    {
        ...
        interfaceSorcerer.RefreshMainScoreUI(newScore, _activeLevel);
        interfaceSorcerer.UpdateLevelDisplay(_activeLevel);

        if (_scoreSystem != null)
        {
            _scoreSystem.ScoreChanged += (score) => interfaceSorcerer.RefreshMainScoreUI(score, _activeLevel);
